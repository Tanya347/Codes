...
class Solution {
  public:
    ...
    int minimizeCost_mem(vector<int>& height, int n, int k, vector<int>& dp) {
        if(n == 1) {
            dp[n] = 0;
            return dp[n];
        }
        ...
        for(int i = 1; i <= k; i++) {
            if(n - i - 1 >= 0)
                ans = min(ans, minimizeCost_mem(height, n - i, k, dp) + abs(height[n - 1] - height[n - i - 1]));
        }
    ...
    int minimizeCost(vector<int>& height, int n, int k) {
        // Code here
        vector<int> dp(n + 1, -1);
        return minimizeCost_mem(height, n, k, dp);
    }
};
