...
public:	
	...
	int findMaxSum(int *arr, int n) {
      ...
      // we can also take another base case n == 2 and return max(arr[0] and arr[1]) instead of this step
	    if(n > 1)
...
int findMaxSum_mem(int* arr, int n, vector<int>& dp) {
	    if(n == 0) {
	        dp[n] = 0;
	        return dp[n];
	    ...
	    if(n > 1)
	        x = arr[n - 1] + findMaxSum_mem(arr, n - 2, dp);
	        
	    int y = findMaxSum_mem(arr, n - 1, dp);
	...
	int findMaxSum(int *arr, int n) {
	    // code here
	    vector<int> dp(n + 1, -1);
	    return findMaxSum_mem(arr, n, dp);
	}
